...
class _JobViewerModalViewState extends State<JobViewerModalView> {
      ...
      if (widget.job.isVacant) {
        grpcFetchJobApplications(AppUser.sessionKey, widget.job).then((res) {
          ...
          if (success) {
            ...
            AppUser.signOut().then(
                (value) => Navigator.of(context).pushReplacementNamed('/'));
        });
      } else
        grpcFetchUserDetails(AppUser.sessionKey, widget.job.hiredUserId)
            .then((res) async {
          bool success = res.item1;
      ...
      childWidgets.addAll([
        ...
        TextField(
            ...
            decoration: InputDecoration(
                ...
                labelText: "Please enter the new vacancy's title here",
                hintText: "e.g., English language teacher")),
        TextField(
            maxLines: 5,
            ...
            decoration: InputDecoration(
                ...
                labelText: "Please enter the new  vacancy's description  here",
                hintText: "e.g.The best position ")),
        TextField(
            controller: _responsibilitiesController,
            decoration: InputDecoration(
                border: new OutlineInputBorder(
                    borderSide: new BorderSide(color: Colors.red)),
                labelText: "Please enter responsibilities here",
                hintText: "e.g.coding, cleaning...")),
        RaisedButton(
            onPressed: () => _createVacantJobPressed(context),
            child: Text("Create a vacant position"))
      ]);
    else {
      // viewing an existing job (read mode)
      childWidgets.addAll([
        getTitleWidget(
            'Viewing a job${widget.job.isVacant ? " / vacancy" : ""}'),
      ...
      if (widget.job.isVacant) {
        ...
        if (_appliedForVacancy != null) {
          if (_appliedForVacancy)
            ...
            childWidgets.add(RaisedButton(
                onPressed: () => _applyForVacancyPressed(context),
                child: Text("Apply for vacancy")));
        }

        // viewing job applications for the position
        if (widget.businessPage != null &&
            widget.businessPage.role == VacancyRole.BUSINESS_OWNER) {
          childWidgets.add(RaisedButton(
              onPressed: () => _viewJobApplicationsPressed(context),
              child: Text("View job applications")));
        }
      } else
        childWidgets.add(Text(
            'Job held by : ${_employeeUser == null ? "[loading]" : _employeeUser.isMe ? "you" : "${_employeeUser.name} (${_employeeUser.email})"}'));

      // edit & delete job (i.e., business owner edits/deletes an employee job/position in his business page)
      if (widget.businessPage != null &&
          widget.businessPage.role == VacancyRole.BUSINESS_OWNER &&
          widget.job.role == VacancyRole.EMPLOYEE)
        childWidgets
  ...
  void _applyForVacancyPressed(BuildContext context) async {
    await showModalBottomSheet(
        context: context,
        builder: (context) => JobApplicationViewerModalView(job: widget.job));
    Navigator.of(context).pop();
  }
  ...
  void _viewJobApplicationsPressed(BuildContext context) async {
    ...
    await Navigator.of(context).pushNamed(
        '/business_page_details/job_applications_list',
        arguments: {'job': widget.job, 'businessPage': widget.businessPage});
    Navigator.of(context).pop();
  }
}
