  ...
  void _calendarMasterButtonPressed() {
    ...
    for (String weekday in _weekdays)
      if (_weekdayEnabled[weekday]) if (completelySelected)
        completelySelected &= widget.timeSlots[weekday].length == 24 - _calendarStartHour;
      else
        break;

    widget.timeSlots.clear();
    if (!completelySelected) for (String weekday in _weekdays) if (_weekdayEnabled[weekday]) widget.timeSlots[weekday] = List<int>.generate(24 - _calendarStartHour, (index) => _calendarStartHour + index).toSet();
  ...
  void _calendarWeekdayPressed(final String day) {
    ...
    if (dayCompletelySelected) {
      ...
      if (widget.timeSlots.containsKey(day))
        for (int hour = _calendarStartHour; hour < 24; hour++) widget.timeSlots[day].add(hour);
      else
        widget.timeSlots[day] = List<int>.generate(24 - _calendarStartHour, (index) => _calendarStartHour + index).toSet();
    }
    setState(() {});
  }
