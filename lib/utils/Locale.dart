...
class Locale {
  ...
  * */
  ...
  static Map<String, List<String>> _stringsMap = {
    ...
    "New business page": ["Новая бизнес страница", "새로운 비즈니스 페이지", "Yangi biznes sahifa"],
    ...
    "Create": ["Создать", "창조하기", "Yaratish"],
    "Update": ["Обновить", "최신 정보", "Yangilash"],
    "Request to publish": ["Запрос на публикацию", "게시 요청", "Tarqatish uchun so'rov"],
    "Unpublish": ["Отменить публикацию", "게시 취소", "Tarqatishdan to'xtatish"],
    ...
    'Candidate details': ['Детали кандидата', '후보자 세부 정보', 'Nomzod tafsilotlari'],
    'Apply to "$REPLACE"': ['Подать заявку на "$REPLACE"', '신청서 "$REPLACE"', '"$REPLACE" uchun ariza to\'ldirish'],
    "Message": ["Сообщение", "메시지", "Habar"],
    ...
    "Attached files": ["Прикрепленные файлы", "첨부 파일", "Biriktirilgan fayllar"],
    ...
    '$REPLACE (business page)': ["$REPLACE (бизнес-страница)", "$REPLACE (비즈니스 페이지)", "$REPLACE (biznes sahifa)"],
    "Create new product": ["Создать новый продукт", "새 제품 만들기", "Yangi mahsulot yaratish"],
    "Create new vacancy": ["Создать новую ваканцию", "새 공석 만들기", "Yangi vakansiya yaratish"],
    "New vacancy": ["Новая ваканция", "새로운 공석", "Yangi vakansiya"],
    ...
    "Candidates for $REPLACE": ["Кандидаты на $REPLACE", "$REPLACE 직업 후보자", "$REPLACE kandidatlari"],
    ...
    "Purchase ( $REPLACE )": ["Купить ( $REPLACE )", "매수 ( $REPLACE )", "Xarid qilish ( $REPLACE )"],
